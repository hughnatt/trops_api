...
router.get('/advert/owner',async (req,res) => { //TODO : Handle owner Token !!
    try{
...
router.delete('/advert/delete',async (req,res) => { //TODO : Handle the user's token !!
    try{
        Advert.findOneAndRemove({_id: req.body._id},function(err,data)
        {
            if(err){
                res.status(400).send(err); 
            }
